...
| 9 | **ROM archive regeneration actually runs** `src/port/Extractor/ExtractFlow.cpp` | The "Outdated ROM Archives" flow deleted the old archive by its bare filename relative to the working directory, which on macOS is not the data folder — so it announced a re-extract every launch and then silently kept using the archive it had just declared incompatible. The delete now resolves through the same lookup that found the archive. |
| 10 | **Native ROM picker** `src/port/FilePickerMac.mm` | The ROM file dialog was an external `osascript` process that could open behind a fullscreen game (an apparently endless beachball) while the game blocked waiting for it. Replaced on macOS with the app's own `NSOpenPanel`, shown asynchronously: the game keeps rendering, fullscreen is undisturbed, and the panel presents above it like any native app's open dialog. |
